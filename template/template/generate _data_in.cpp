...
for (const auto& transition : net.transitions) {
    ...
    if (transition == "T_11") {
        // Special case for T_11: Add "data" as an input and "data_11" as output
    ...
    // Define function_args (including control input)
    ...
    if (transition == "T_11") {
        function_args.push_back("data"); // Additional input for T_11
        for (const auto& output : outputs) {
    ...
    if (transition == "T_0") {
        ...
        std::string data_ref = is_T_11 ? "data" : outputs[0]; // Use "data" for T_11, output place otherwise
        ...
        xml << "                        std::cout << \"j = \" << j << \", k = \" << k << std::endl;\n";
            ...
            xml << "                        std::string reducedIBPs_seed = RESOLVE_INTERFACE_FUNCTION(singular_seed_gpi)("
                << data_ref << ".reducedIBPs, library_name, base_filename);\n";
            ...
            xml << "                        std::string wjk = RESOLVE_INTERFACE_FUNCTION(singular_replace_two)("
                << data_ref << ".web, j, k, library_name, base_filename);\n";
            xml << "                        std::string wjk_targetInt = RESOLVE_INTERFACE_FUNCTION(singular_targetInts_gpi)(wjk, library_name, base_filename);\n";
            xml << "                        std::cout << \"web[\" << j << \",\" << k << \"].TARGET = \" << RESOLVE_INTERFACE_FUNCTION(printGpiTokenContent)(wjk_targetInt, library_name) << std::endl;\n";
            ...
            xml << "                            std::string w = RESOLVE_INTERFACE_FUNCTION(singular_updateWeb_later)("
                << data_ref << ".web, masterAndTailIntgrals, j, k, library_name, base_filename);\n";
            xml << "                            std::string w1 = RESOLVE_INTERFACE_FUNCTION(singular_Return_place)(w, 1, library_name, base_filename);\n";
            xml << "                            " << data_ref << ".web = RESOLVE_INTERFACE_FUNCTION(singular_assign_gpi)(w1, library_name, base_filename);\n";
            ...
            xml << "                            int MI_size = RESOLVE_INTERFACE_FUNCTION(singular_size_gpi)("
                << data_ref << ".MI, library_name, base_filename);\n";
            xml << "                            if (MI_size == 0) {\n";
            xml << "                                " << data_ref << ".MI = RESOLVE_INTERFACE_FUNCTION(singular_assign_gpi)(masterIntgralsInSector, library_name, base_filename);\n";
            ...
            xml << "                                    int alreadyIntheList = RESOLVE_INTERFACE_FUNCTION(singular_alreadyIntheList_gpi)("
                << data_ref << ".MI, masterIntgralsInSector_i, library_name);\n";
            xml << "                                    if (!alreadyIntheList) {\n";
            xml << "                                        " << data_ref << ".MI = RESOLVE_INTERFACE_FUNCTION(singular_assign_gpi)(masterIntgralsInSector_i, library_name, base_filename);\n";
        ...
        // Connections
        if (data_inputs.size() == 1 && transition != "T_end" && outputs.size() == 1) {
            std::string output_place = outputs[0];
